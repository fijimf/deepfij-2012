...
object ModelTester {
  ...
  def main(args: Array[String]) {
    ...
    val atop20: List[(Team, Double)] = statistics("wp").population(new SimpleDateFormat("yyyyMMdd").parse("20120121")).topN(20)
    val btop20: List[(Team, Double)] = statistics("wp").population(new SimpleDateFormat("yyyyMMdd").parse("20120221")).topN(20)
    val ctop20: List[(Team, Double)] = statistics("wp").population(new SimpleDateFormat("yyyyMMdd").parse("20120321")).topN(20)
